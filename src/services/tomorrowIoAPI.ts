...
export async function fetchTomorrowIoData(
  location: string
): Promise<tomorrowIoDTO> {
  ...
  return {
    ...
    probabilityOfPrecipitation:
      json.timelines.minutely[0].values.precipitationProbability,
    ...
    weatherCode: json.timelines.minutely[0].values.weatherCode,
  };
}
...
export async function fetchTomorrowIoDataByGeo(
  lat: number,
  lon: number
): Promise<tomorrowIoDTO> {
  const url = `https://api.tomorrow.io/v4/weather/forecast?lat=${lat}&lon=${lon}&units=metric&apikey=A9jACd3dteXNmKj7fUM63wSJsrJroRpn`;
  const options = {
    method: "GET",
    headers: {
      accept: "application/json",
